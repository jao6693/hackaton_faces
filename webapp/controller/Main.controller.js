sap.ui.define([
	...
	return Controller.extend("dlw.hackaton.controller.Main", {
		...
		handleValueChange: function(oEvent) {
			...
			var rgpd = settings.getProperty("/rgpd");
			...
			if(oEvent.getParameters().files[0] !== undefined) {
				...
				this.generateRequest(i, urls[i],rgpd, oView, oBusyIndicator);
		...
		generateRequest: function(i, service, rgpd, oView, oBusyIndicator) {
			var that = this;
			...
			$.ajax({
				...
				try {
					...
					for (var j = 0; j < response.predictions[0].results.length; j++) {
						if (response.predictions[0].results[j].score > 0.1) {
							model.results[j].score[i] = Math.round(response.predictions[0].results[j].score * 1000) / 10;
						} else {
							// do not include the result in the dashboard
						...
						// add a color
						model.results[j].color = [];
						model.results[j].color.push("Good");
						for (var k = 0; k < rgpd.length; k++) {
							if (rgpd[k].name == model.results[j].label[i]) {
								model.results[j].color[i] = rgpd[k].color;
							}
						}
					}
					oView.getModel().refresh();
